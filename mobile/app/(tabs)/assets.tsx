...
function HistoryView({ history }: { history: HistoryPoint[] }) {
  const { th, fmt, t, privacyMode, historyBackfilling } = useApp();
  const blur = privacyMode ? '••••' : null;
  const { width } = useWindowDimensions();
  const data = windowByDays(history, 60, Date.now());
  if (historyBackfilling) {
    return (
      <View style={{ alignItems: 'center', paddingVertical: 40 }}>
        <ActivityIndicator size="small" color={th.acc} />
        <Text style={{ color: th.tx3, marginTop: 12, textAlign: 'center' }}>
          {t('asset_history_loading')}
        </Text>
      </View>
    );
  }
